// Local-Only Image Optimization System
// ระบบ Optimize รูปภาพแบบมืออาชีพ - ใช้ไฟล์ local เท่านั้น
...
function readVillaData() {
  ...
  return data.map((row, index) => {
    ...
    const bedrooms = parseInt(row['Bedroom']) || 3; // Default to 3 if missing
    ...
    const avgPrice = prices.length > 0 
      ...
      : Math.max(bedrooms * 5000, 15000); // Minimum 15,000 baht
...
function saveOptimizedImage(buffer, villaSlug, category, filename) {
  ...
  return `/optimized-villas/${villaSlug}/${category}/${sanitizePublicId(baseName)}.webp`;
}
...
async function processImage(image, villa) {
  ...
  try {
    ...
    // 2. Save locally and get web path
    const webPath = saveOptimizedImage(
      optimized.webp,
      villa.slug,
      image.category,
      image.filename
    ...
    return {
      ...
      url: webPath,
      localPath: path.join(CONFIG.outputDir, villa.slug, image.category, `${sanitizePublicId(path.parse(image.filename).name)}.webp`),
...
async function processVilla(villa, villaIndex, totalVillas) {
  ...
  // Process in batches
  for (let i = 0; i < images.length; i += CONFIG.processing.batchSize) {
    const batch = images.slice(i, i + CONFIG.processing.batchSize);
    ...
    if (i + CONFIG.processing.batchSize < images.length) {
      await sleep(CONFIG.processing.delayMs);
    }
  }
...
async function main(options = {}) {
  ...
  console.log('🚀 Local-Only Image Optimization System');
  if (dryRun) console.log('🧪 DRY RUN MODE');
  console.log('📁 Output: public/optimized-villas/');
  console.log('🚫 No Cloudinary upload - Local files only');
  console.log('═'.repeat(60));
  ...
  console.log(`\n💾 Local files: ${CONFIG.outputDir}`);
  console.log(`📄 JSON files: data/villas-optimized.json`);
  console.log('\n' + '═'.repeat(60));
}
