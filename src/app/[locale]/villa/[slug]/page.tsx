...
async function getVilla(slug: string, locale: string): Promise<Villa | null> {
  try {
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3001';
    // ใช้ villa API ที่มีอยู่แล้ว แล้วค้นหา by slug
    const response = await fetch(`${baseUrl}/${locale}/api/villas?limit=1000`, {
      cache: 'no-store'
    });
...
export async function generateMetadata({ params }: { params: { slug: string; locale: string } }): Promise<Metadata> {
  ...
  const villa = await getVilla(resolvedParams.slug, resolvedParams.locale);
...
export default async function VillaDetailPage({ params }: { params: { slug: string; locale: string } }) {
  const resolvedParams = await params;
  const villa = await getVilla(resolvedParams.slug, resolvedParams.locale);
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <Link href={`/${resolvedParams.locale}/booking/${villa.slug}`}>
