...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedVillas = villas.map(villa => {
      // Get current month pricing (prefer current month, fallback to any available)
      const now = new Date();
      const currentMonth = now.getMonth() + 1; // 1-12
      const currentYear = now.getFullYear();
      
      // Find pricing for current month or latest available
      const currentPricing = villa.pricing?.find(p => 
        p.month === currentMonth && (p.year === null || p.year === currentYear)
      ) || villa.pricing?.[0] || null;
      ...
      const baseVilla = {
        ...
        weeklyRate: currentPricing?.weeklyRate ? Number(currentPricing.weeklyRate) : null,
        monthlyRate: currentPricing?.monthlyRate ? Number(currentPricing.monthlyRate) : null,
        pricing: currentPricing ? {
          dailyRate: Number(currentPricing.dailyRate),
          weeklyRate: currentPricing.weeklyRate ? Number(currentPricing.weeklyRate) : null,
          monthlyRate: currentPricing.monthlyRate ? Number(currentPricing.monthlyRate) : null,
          currency: currentPricing.currency,
          month: currentPricing.month,
          year: currentPricing.year
        } : null,
